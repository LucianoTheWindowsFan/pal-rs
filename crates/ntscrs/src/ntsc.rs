...
mod pal_param {
    pub const chroma_subcarrier: f32 = 1,
    pub const v_bandwidth: f32 = 2,
}
...
const NTSC_RATE: f32 = (pal_param::chroma_subcarrier / 88.0) * 4.0;
...
fn composite_chroma_lowpass(frame: &mut YiqView, info: &CommonInfo, filter_type: FilterType) {
    ...
    let q_filter = make_lowpass_for_type(pal_param::v_bandwidth, NTSC_RATE * info.bandwidth_scale, filter_type);
    ...
    fn apply_effect_to_yiq_field(&self, yiq: &mut YiqView, frame_num: usize) {
        ...
        if self.composite_preemphasis > 0.0 {
            let preemphasis_filter = make_lowpass(
                (pal_param::chroma_subcarrier / 88.0 / 2.0) * self.bandwidth_scale,
